...
const ContractorInfo = ({ contractor, ...props }) => {
  const [isOpen, setIsOpen] = useState(false)
  const handleClose = () => setIsOpen(false)
  const handleOpen = () => setIsOpen(true)

  const [contractorLive, setContractorLive] = React.useState({...contractor})
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-2xl transform overflow-y-auto rounded-xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  {!loading ? (
                    <ContractorLayout contractor={contractorLive} />
                  ) : (
