...
const ArtistForm = ({ type, defaultValues, onFormSubmit, ...props }) => {
  ...
  useEffect(() => {
    if (defaultValues) {
      ...
      setValue(
        'genres',
        defaultValues?.genres?.map((genre) => genre.slug) || []
      )
      ...
      setValue(
        'bandMembers',
        defaultValues.bandMembers.map((member) => member.slug)
      )
    }
  ...
  useEffect(() => {
    axios
      .get(getBaseUrl() + `/artists?search=` + search)
      .then((res) => {
    ...
    <div {...props} className="flex flex-col space-y-6">
      <form>
        <FormSection defaultOpen={true} title={'Artist Information'}>
          ...
          <div className="flex flex-col gap-4">
            ...
            {errors.primaryImageFile && (
              <p>{errors.primaryImageFile.message}</p>
            )}
      ...
      <FormSection title={'More Info'}>
        ...
        <Select
          ...
          error={
            errors.dominantLanguage ? errors.dominantLanguage.message : false
          }
      ...
      <FormSection title={'Band Information'}>
        <RadioSelect
          ...
          error={errors.isBand ? errors.isBand.message : false}
        />
