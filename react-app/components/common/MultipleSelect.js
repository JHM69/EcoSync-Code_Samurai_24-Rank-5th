...
const MultipleSelect = ({ name, label, register, children, error, className, ...props }) => {
  ...
  return (
    <div className="mb-2">
      ...
      <select
        className={clsx([
          'form-control block w-full h-[250px] border border-solid bg-white bg-clip-padding px-4 py-2 font-normal text-gray-700 focus:ring-2',
...
const OptionWithCheckbox = ({ value, children, register, multiple }) => (
  <option value={value} {...register} className="flex items-center">
    ...
    type="checkbox" 
    value={value} 
    className="mx-2 rounded-xl p-2 hover:bg-green-100"
    />}
    {children}
  </option>
