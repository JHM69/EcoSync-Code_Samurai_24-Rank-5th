// @ts-nocheck
const getHtml = (
    entryid,
    billCreatedAt,
    timeOfArrival,
    timeOfDeparture,
    volumeOfWaste,
    truckId,
    truckRegistrationNumber,
    truckType,
    unloadedFuelCost,
    loaddedFuelCost,
    capacity,
    cost,
    assigneeId
  )=>{
    billCreatedAt = `${new Date(billCreatedAt).toLocaleTimeString()} - ${new Date(billCreatedAt).toLocaleDateString()}`;
    timeOfArrival = `${new Date(timeOfArrival).toLocaleTimeString()} - ${new Date(timeOfArrival).toLocaleDateString()}`;
    timeOfDeparture = `${new Date(timeOfDeparture).toLocaleTimeString()} - ${new Date(timeOfDeparture).toLocaleDateString()}`;
    cost = cost.toFixed(2);

    return `<!DOCTYPE html>
  <html>
    <head>
      <meta charset="utf-8" />
      <title>A simple, clean, and responsive HTML invoice template</title>
  
      <style>
        .invoice-box {
          max-width: 800px;
          margin: auto;
          padding: 30px;
          border: 1px solid #eee;
          box-shadow: 0 0 10px rgba(0, 0, 0, 0.15);
          font-size: 16px;
          line-height: 24px;
          font-family: "Helvetica Neue", "Helvetica", Helvetica, Arial, sans-serif;
          color: #555;
        }
  
        .invoice-box table {
          width: 100%;
          line-height: inherit;
          text-align: left;
        }
  
        .invoice-box table td {
          padding: 5px;
          vertical-align: top;
        }
  
        .invoice-box table tr td:nth-child(2) {
          text-align: right;
        }
  
        .invoice-box table tr.top table td {
          padding-bottom: 20px;
        }
  
        .invoice-box table tr.top table td.title {
          font-size: 45px;
          line-height: 45px;
          color: #333;
        }
  
        .invoice-box table tr.information table td {
          padding-bottom: 40px;
        }
  
        .invoice-box table tr.heading td {
          background: #eee;
          border-bottom: 1px solid #ddd;
          font-weight: bold;
        }
  
        .invoice-box table tr.details td {
          padding-bottom: 20px;
        }
  
        .invoice-box table tr.item td {
          border-bottom: 1px solid #eee;
        }
  
        .invoice-box table tr.item.last td {
          border-bottom: none;
        }
  
        .invoice-box table tr.total td:nth-child(2) {
          border-top: 2px solid #eee;
          font-weight: bold;
        }
  
        @media only screen and (max-width: 600px) {
          .invoice-box table tr.top table td {
            width: 100%;
            display: block;
            text-align: center;
          }
  
          .invoice-box table tr.information table td {
            width: 100%;
            display: block;
            text-align: center;
          }
        }
  
        /** RTL **/
        .invoice-box.rtl {
          direction: rtl;
          font-family: Tahoma, "Helvetica Neue", "Helvetica", Helvetica, Arial,
            sans-serif;
        }
  
        .invoice-box.rtl table {
          text-align: right;
        }
  
        .invoice-box.rtl table tr td:nth-child(2) {
          text-align: left;
        }
      </style>
    </head>
  
    <body>
      <div class="invoice-box">
        <table cellpadding="0" cellspacing="0">
          <tr class="top">
            <td colspan="2">
              <table>
                <tr>
                  <td class="title">
                    <img
                      src="https://i.ibb.co/6YpTrN5/ecosync.png"
                      style="width: 30%; max-width: 300px"
                    />
                  </td>
  
                  <td>
                    Invoice #: ${entryid} <br/>
                    Created At: ${billCreatedAt} <br/>
                  </td>
                </tr>
              </table>
            </td>
          </tr>
  
          <tr class="information">
            <td colspan="2">
              <table>
                <tr>
                  <td>
                    EcoSync - Quantum Guys<br />
                    Dhaka North City Corporation<br />
                  </td>
                </tr>
              </table>
            </td>
          </tr>
  
          <tr class="heading">
            <td>Entry</td>
            <td>#</td>
          </tr>
  
          <tr class="item">
            <td>Time Of Arrival</td>
  
            <td>${timeOfArrival}</td>
          </tr>
          <tr class="item">
            <td>Time Of Departure</td>
  
            <td>${timeOfDeparture}</td>
          </tr>
          <tr class="details">
            <td>Weight</td>
  
            <td>${volumeOfWaste} TON</td>
          </tr>
  
          <tr class="heading">
            <td>Truck Details</td>
            <td>#</td>
          </tr>
  
          <tr class="item">
            <td>Truck Id</td>
            <td>${truckId}</td>
          </tr>
  
          <tr class="item">
            <td>Registration Number</td>
            <td>${truckRegistrationNumber}</td>
          </tr>
  
          <tr class="item">
            <td>Truck Type</td>
            <td>${truckType}</td>
          </tr>
          <tr class="details">
            <td>Capacity</td>
            <td>${capacity} TON</td>
          </tr>
  
          <tr class="heading">
            <td>Cost per KM</td>
            <td>#</td>
          </tr>
  
          <tr class="item">
            <td>Unloaded</td>
            <td>BDT ${unloadedFuelCost}</td>
          </tr>
  
          <tr class="item last">
            <td>Full loaded</td>
            <td>BDT ${loaddedFuelCost}</td>
          </tr>
  
          <tr class="total">
            <td></td>
  
            <td>Total: ৳ ${cost}</td>
          </tr>
        </table>
      </div>
    </body>
  </html>`;
  }
  
  export { getHtml};
  